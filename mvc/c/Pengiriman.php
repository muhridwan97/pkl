...
class Pengiriman extends CI_Controller {
	...
	global $hargaKartuS1,$hargaKartuS2;
		...
		if (count($cek)>0){
			...
			$tglLulus = $this->ubahTanggal($data['tglLulus']);
			
			if($data['tglKerja']==""){
				$data['tglKerja']="01/01/2000";
			}
			$tglKerja = $this->ubahTanggal($data['tglKerja']);
			
			$start_date = new DateTime($tglLulus);
			$start_date2 = new DateTime($tglKerja);
			$end_date = new DateTime(); // tanggal sekarang berdasarkan tanggal di komputer
			$selisihLulus        = $end_date->diff($start_date)->format("%a");
			$selisihKerja        = $end_date->diff($start_date2)->format("%a");
			
			$selisihLulus = (int)$selisihLulus/30;
			$selisihKerja = (int)$selisihKerja/30;
			
			if($selisihLulus>=24 || $selisihKerja>=12){
